...
from backend.utils import save_results, save_submission
from backend.scorer import (
    final_score,
    candidate_assessment,
...
# -------------------------
    ...
    for candidate in candidates:
        ...
        skill = calculate_skill_score(job_text, candidate)
        experience = calculate_experience_score(candidate)

        quick_score = skill * 0.7 + experience * 0.3

        filtered_candidates.append((quick_score, candidate))
    ...
    candidates = [
        candidate
        for _, candidate in filtered_candidates[:500]
    ]
...
if "ranked" in st.session_state:
    ...
    submission_file = save_submission(ranked)
